...
import indianLocations from '../../../indianlocations.json'; // Importing JSON file
...
const Header = ({ setCoordinates }) => {
    ...
    useEffect(() => {
        // Extracting city names from the JSON file
        const cityNames = indianLocations.map(location => location.city);
    ...
    const handleInputChange = (event) => {
        ...
        const filteredSuggestions = Array.from(suggestions).filter(cityName =>
    ...
    const handleSuggestionClick = (cityName) => {
        const selectedCity = indianLocations.find(location => location.city === cityName);
        if (selectedCity) {
            const { latitude, longitude } = selectedCity;
            setCoordinates({ lat: latitude, lng: longitude });
        }
        setInputValue(cityName);
        setSuggestions(new Set()); // Clear suggestions
    };
    return (
